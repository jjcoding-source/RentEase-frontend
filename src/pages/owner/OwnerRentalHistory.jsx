...
export default function OwnerRentalHistory() {
  ...
  const { data: history = [], isLoading } = useQuery({
    queryKey: ['owner-rental-history'],
    queryFn:  () => api.get('/bookings/owner/history').then(r => r.data),
  })

  const totalRevenue = history.reduce((s, h) => s + (h.rent || 0) * (h.duration || 0), 0)
    ...
    <PageWrapper>
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-[12px] text-[#727787] mt-0.5">All completed rentals across your properties</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        ...
        <StatCard label="Total revenue"       value={formatINR(totalRevenue)} valueColor="text-[#0053cc]" />
      </div>

      {/* Table */}
      {isLoading && (
        <div className="flex flex-col gap-3">
          {[1,2,3].map(i => <div key={i} className="bg-white border border-[#e6e7f4] rounded-xl h-16 animate-pulse" />)}
        </div>
      )}
      ...
      {!isLoading && history.length > 0 && (
        <div className="bg-white border border-[#e6e7f4] rounded-xl overflow-hidden">
          ...
          <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
            ...
            <tbody>
              {history.map((h, i) => (
                <tr key={h.id} className="border-b border-[#f2f3ff] hover:bg-[#faf8ff] last:border-0">
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-9 h-7 rounded bg-gradient-to-br ${CARD_GRADIENTS[i % 4]} flex-shrink-0`} />
                      <div>
                        <div className="text-[12px] font-semibold text-[#191b24] leading-snug">{h.propertyTitle}</div>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-[12px] font-medium text-[#191b24] truncate">{h.renterName}</div>
                    </div>
                  </td>
