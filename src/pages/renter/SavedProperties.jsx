...
export default function SavedProperties() {
  ...
  const { mutateAsync: unsave } = useMutation({
    mutationFn: (id) => api.delete(`/properties/${id}/save`),
    ...
    <PageWrapper savedCount={saved.length}>
      ...
      {!isLoading && saved.length === 0 && (
        <div className="bg-white border border-[#e2e8f0] rounded-xl py-16 text-center">
          ...
          <div className="text-[14px] font-semibold text-[#334155] mb-1">No saved properties yet</div>
          <div className="text-[12px] text-gray-400 mb-4">Heart a property to save it here for later.</div>
          <button
            onClick={() => navigate('/properties')}
      ...
      {!isLoading && saved.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {saved.map((p, i) => (
            <div key={p.id} className="bg-white border border-[#e2e8f0] rounded-xl overflow-hidden hover:border-[#bfdbfe] hover:shadow-sm transition-all">
              ...
              <div className={`h-36 bg-gradient-to-br ${CARD_GRADIENTS[i % CARD_GRADIENTS.length]} relative`}>
                ...
                <button
                  onClick={() => unsave(p.id)}
                  className="absolute top-2 right-2 w-6 h-6 bg-white/95 rounded-full flex items-center justify-center text-[13px] text-red-500 border border-[#e2e8f0] hover:scale-110 transition-transform"
                >
                  ♥
                </button>
                {p.availableNow && (
              ...
              <div className="p-3">
                ...
                <div className="text-[13px] font-semibold text-[#1e293b] mb-0.5 truncate">{p.title}</div>
                ...
                {p.amenities?.length > 0 && (
                  <div className="flex gap-1 flex-wrap mb-2.5">
                    {p.amenities.slice(0, 3).map(a => (
                      <span key={a} className="text-[9px] bg-[#eef4ff] text-[#1558c0] px-1.5 py-0.5 rounded-full font-semibold">{a}</span>
                    ))}
                  </div>
                )}
